...
def print_cluster_keywords(clusterer, vectorizer, k_index):
    """
    takes: TextClusterer, TextVectorizer, k_index used for clustering
    """
...
def calculate_rand(size, clustered, training_data_stripped):
    """
    takes: size of set, Array of clustered data, DataFrame
    returns: rand index as number
    """
    pos_count = 0
...
def find_optimal_kindex(clusterer, training_bodies, training_data_stripped):
    """
    takes: TextClusterer, Array of bodies in training data, DataFrame
    returns: rand index as number
    """
    k_index = range(3, 150)
    print(f"Index\t\tRand")
            ...
            for j in range(i, 500):
                if i != j:
                    if clustered[i] == clustered[j]:  # Same cluster and same category
                        if training_data_stripped['Path'][i] == training_data_stripped['Path'][j]:
                            pos_count += 1
                    # Different cluster and different category
...
if __name__ == '__main__':
    ...
    training_data_stripped['Path'] = pd.Series([[path[0], path[1]] if len(
        path) > 1 else None for path in training_data_stripped['Path']]).values
    ...
    for eps_val in eps_vals:
        clusterer_comp = DBSCAN(eps=eps_val)
        clustered_comp = clusterer_comp.fit_predict(training_bodies)
        rand_index = calculate_rand(
            500, clustered_comp, training_data_stripped)
